    ...
    for k in range(len(person["T"]) - 1):
        ...
        if len(charge_pos) > 1:
            for u in range(len(charge_pos) - 1):
                total_distance = total_distance + Network_Frame.distance(charge_pos[d[u]], charge_pos[d[u + 1]])
        total_distance = total_distance + Network_Frame.distance(charge_pos[d[0]],
                                                                 Network_Frame.depot) + Network_Frame.distance(
            charge_pos[d[-1]], Network_Frame.depot)
        time_move = total_distance / Network_Frame.velocity
        total = total + time_move + person["T"][k] + sum(person["x"][k])
    x = person["x"][-1]
    d = person["d"][-1]
    total = total + person["T"][-1]
    for u, du in enumerate(d):
        if x[d[u]] == 0:
            break
        if u == 0:
            time_to_move = Network_Frame.distance(Network_Frame.depot, charge_pos[d[u]]) / Network_Frame.velocity
        else:
            time_to_move = Network_Frame.distance(charge_pos[d[u - 1]], charge_pos[d[u]]) / Network_Frame.velocity
        total = total + time_to_move + x[d[u]]
...
def repair(person):
    ...
    for k in range(len(person["T"])):
        ...
        eNode = min(
            [temp_E[j] - Network_Frame.distance(Network_Frame.depot, charge_pos[d[0]]) * Network_Frame.e[
        ...
        if eNode < 0 or temp_E_mc < E_move:
            ...
            for u, du in enumerate(d):
                if u == 0:
                    time_move = Network_Frame.distance(Network_Frame.depot, charge_pos[d[u]]) / Network_Frame.velocity
                else:
                    time_move = Network_Frame.distance(charge_pos[d[u - 1]], charge_pos[d[u]]) / Network_Frame.velocity
                eNode = min([E_now[j] - time_move * Network_Frame.e[j] for j, _ in enumerate(node_pos)])
                if eNode < 0:
                    isStop = True
                    break
                p = [min(Network_Frame.charging(node, charge_pos[d[u]]) * x[d[u]],
                         Network_Frame.E[j] - E_now[j] + (time_move + x[d[u]]) * Network_Frame.e[j]) for j, node in
                     enumerate(node_pos)]
                temp_E_mc = E_mc_now - sum(p) - time_move * Network_Frame.e_move
                temp_E = [E_now[j] + p[j] - (time_move + x[d[u]]) * Network_Frame.e[j] for j, _ in enumerate(node_pos)]
            ...
            if not isStop:
                E_mc_now = E_mc_now - Network_Frame.distance(Network_Frame.depot, charge_pos[
                    d[-1]]) * Network_Frame.e_move / Network_Frame.velocity
                E_now = [E_now[j] - Network_Frame.distance(Network_Frame.depot, charge_pos[d[-1]]) * Network_Frame.e[
                    j] / Network_Frame.velocity for j, _ in enumerate(node_pos)]
            else:
                break
    if off["remain"] == -1:
...
def individual(p_rand, is_sorted):
    person = {"location": locate_init(is_sorted=is_sorted), "T": [], "x": [], "d": [], "remain": 0, "fitness": 0}
    r = random.random()
    if r <= p_rand:
    ...
    person["T"].append(T)
    person["x"].append(x)
    d = [i for i in range(len(person["location"]))]
    random.shuffle(d)
    person["d"].append(d)
...
def crossover(father, mother, has_hungary):
    child = {"location": blx_pair(father=father["location"], mother=mother["location"], has_hungary=has_hungary),
             ...
             "x": [blx_scalar(father=father["x"][i], mother=mother["x"][i]) for i, _ in enumerate(father["x"]) if
                   i < len(mother["x"])],
             "d": [ox_cross(father=father["d"][i], mother=mother["d"][i]) for i, _ in enumerate(father["d"]) if
                   i < len(mother["d"])],
             "remain": 0, "fitness": 0}
    x_index = len(child["x"])
    ...
    for k, _ in enumerate(off["x"]):
        ...
        for u, du in enumerate(d):
            if u == 0:
                ...
                time_move = Network_Frame.distance(charge_pos[d[u - 1]], charge_pos[d[u]]) / Network_Frame.velocity
            p = [min(Network_Frame.charging(node, charge_pos[d[u]]) * x[d[u]],
                     Network_Frame.E[j] - E_now[j] + (time_move + x[d[u]]) * e[j]) for j, node in enumerate(node_pos)]
            E_mc_now = E_mc_now - sum(p) - time_move * Network_Frame.e_move
            E_now = [E_now[j] + p[j] - (time_move + x[d[u]]) * e[j] for j, _ in enumerate(node_pos)]
        E_mc_now -= Network_Frame.distance(Network_Frame.depot,
                                           charge_pos[d[-1]]) * Network_Frame.e_move / Network_Frame.velocity
        E_now = [
            E_now[j] - Network_Frame.distance(Network_Frame.depot, charge_pos[d[-1]]) * e[j] / Network_Frame.velocity
...
def genetic(start, end, p_cross, p_mutate, m_rand, connection):
    global population
    sub_pop = []
    count = 0
    i = start
    while i < end:
        ...
        if rc <= p_cross:
            ...
            mutated_child = mutation(child, m_rand)
            if mutated_child != -1:
                count += 1
        ...
        if rm <= p_mutate:
            mutated_child = mutation(population[i], m_rand)
            if mutated_child != -1:
                count += 1
                sub_pop.append(mutated_child)
        i += 1
...
def evolution(maxIterator, p_cross, p_mutate, m_rand):
    global population
    bestFitness = 0.0
    nbIte = 0
    t = 0
    ...
    while t < maxIterator and nbIte < 200:
        ...
        for pid in range(nproc):
            pro = Process(target=genetic, args=(5 * pid, 5 * (pid + 1), p_cross, p_mutate, m_rand, connection[pid][1]))
            process.append(pro)
            pro.start()
        for pid in range(nproc):
...
while index < 1:
    ...
    for idRun in range(nbRun):
        ...
        indi, conv = evolution(maxIterator=5000, p_cross=0.8, p_mutate=0.5, m_rand=0.5)
        end_time = time.time()
        #  tong hop ket qua
