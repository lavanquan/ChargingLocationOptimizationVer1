    ...
    for index, x in enumerate(person["x"]):
        ...
        if eNode < 0 or temp_E_mc < E_move:
            ...
            for u, xu in enumerate(x):
                ...
                eNode = min([E_now[j] - time * Network_Frame.e[j] for j, _ in enumerate(node_pos)])
                if eNode < 0:
                    isStop = True
                    break
                p = [min(Network_Frame.charging(node, charge_pos[u]) * xu,
                         Network_Frame.E[j] - E_now[j] + (time + xu) * Network_Frame.e[j]) for j, node in
                     enumerate(node_pos)]
                temp_E_mc = E_mc_now - sum(p) - time * Network_Frame.e_move
                temp_E = [E_now[j] + p[j] - (time + xu) * Network_Frame.e[j] for j, _ in enumerate(node_pos)]
